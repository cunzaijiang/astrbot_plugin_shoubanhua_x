...
class ShoubanhuaPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        # 注册 WebUI API
        try:
            self.web_api_handler = WebApiHandler(self)
            self.web_api_handler.register_routes()
            logger.info("[Shoubanhua] WebUI API 扩展路由注册成功")
        except Exception as e:
            logger.warning(f"[Shoubanhua] 注册 WebUI API 异常: {e}")
    ...
    async def initialize(self):
        ...
        logger.info(
            f"手办化 插件已加载 v3.1.3 | LLM智能判断: {auto_detect_status} | 上下文轮数: {self._context_rounds}")
    ...
    def _save_config(self, changed_keys: Optional[List[str]] = None):
        ...
        try:
            ...
            # 1. 尝试通过 AstrBot 原生接口保存 (AstrBotConfig.save_config)
            saved = False
            try:
                ...
                logger.warning(f"FigurinePro: AstrBot 原生配置接口保存失败: {e}")
            ...
            try:
                ...
                if astrbot_config_path and os.path.exists(os.path.dirname(astrbot_config_path)):
                    ...
                    if "image_storage_max_gb" in conf_dict:
                        v = conf_dict["image_storage_max_gb"]
                        conf_dict["image_storage_max_gb"] = str(int(float(v))) if isinstance(v, (int, float, str)) and str(v).strip() else "5"
    ...
    async def _run_background_task(self, event: AstrMessageEvent, images: List[bytes],
        ...
        """
        try:
            # 1. 扣费（生成成功后如果不需要退费；若失败且扣了费，在 error 分支退费）
            if charge_quota:
            ...
            if isinstance(res, bytes):
                ...
                if charge_quota:
                    if deduction["source"] == "user":
                        await self.data_mgr.add_user_count(uid, cost)
                    elif deduction["source"] == "group":
                        await self.data_mgr.add_group_count(gid, cost)
    ...
    async def text_to_image_tool(self, event: AstrMessageEvent, prompt: str, count: int = 1,
        ...
        【@他人 / 涉及群友时的生图规则（极度重要，严禁违规！）】
        - 当用户消息中提到了或 @ 了群友/用户（例如：“画一张 @某人 在开卡丁车”、“画一张 @A 偷吃 @B 的小鱼干”）：
          1. 系统后台会自动提取被 @ 用户的头像作为该角色的外貌参考图。
          2. 【严禁脑补外貌】：绝对不要在 prompt 中自行猜测或描写该人物的长相、容貌、性别、发型发色、体型、画风等任何外表特征！严禁擅自描写“一个帅气的男孩”、“留着短发的少年”等细节，否则会导致画出来的图与用户头像完全不符！
          3. 【主体指代与动作】：在 prompt 中，必须将该人物指代为“参考图中的角色/人物”（英文统一用 the character from reference image），只描述TA在画面中执行的动作、姿态、表情、互动与所处的环境场景。若同时涉及多位 @用户，按提及顺序指代为 Character 1 (from reference image 1), Character 2 (from reference image 2)...，仅描述他们之间的动作与互动情节。
    ...
    async def image_edit_tool(self, event: AstrMessageEvent, prompt: str, use_message_images: bool = True,
        ...
        【@他人 / 涉及群友时的图片处理规则（极度重要！）】
        - 当用户请求涉及 @其他用户 时，系统后台会自动提取被 @ 用户的头像作为参考图。
        - 在 prompt 中【严禁】脑补或描写人物长相、性别、发型发色等外貌特征！必须直接将人物指代为 the character from reference image，只描述动作、姿态、表情与场景要求。
    ...
    async def on_txt2img(self, event: AstrMessageEvent, ctx=None):
        ...
        if isinstance(res, bytes):
            ...
            if deduction["source"] == "user":
                await self.data_mgr.add_user_count(uid, 1)
            elif deduction["source"] == "group":
                await self.data_mgr.add_group_count(event.get_group_id(), 1)
    ...
    async def smart_generate_tool(self, event: AstrMessageEvent, user_request: str = ""):
        ...
        如果用户提到了或 @ 了群友，系统会自动抓取其头像作为外貌参考，分析任务或生成提示词时【严禁】脑补描写人物的长相、发型、性别等外貌细节，只关注动作和场景。
    ...
    async def on_persona_photo_cmd(self, event: AstrMessageEvent, ctx=None):
        ...
        if isinstance(res, bytes):
            ...
            if deduction["source"] == "user":
                ...
                await self.data_mgr.add_group_count(gid, 1)
            yield event.chain_result([Plain(f"没弄好: {self._resolve_debug_error_message(res, '这次没弄好，请稍后再试。')}")])
